...
{
	private static final String LCAT = "Tiandroidaudioplayer";
	...
	public MediaPlayerWrapper(KrollProxy proxy) throws IOException
	{
		...
		{
			Log.d(LCAT,"INITIALIZE WITH URL :" + url);
			this.initialize();			
		}
	}
		...
		throws IOException
	{
		try {
			...
			if (URLUtil.isAssetUrl(url)) {
				Log.d(LCAT,"audio is ASSET url." + url);
				...
				if (uri.getScheme().equals(TiC.PROPERTY_FILE)) {
					...
					Log.d(LCAT,"audio is a FILE url." + url);
...
//					See: https://code.google.com/p/streamscraper/ for more info. 
//					But it's very old code. 
// 					NOTE: timedMetaData is available from API Level 23. 
...
//					Log.d(LCAT,"audio URL. " + songUrl);
			...
			if(remote){ // try async
				...
				// add the listener here too, why not.
				mp.setOnPreparedListener(this);
	...
	public void onBufferingUpdate(MediaPlayer mp, int percent)
	{
		...
		// Seems to still fire after 100

		KrollDict data = new KrollDict();
		data.put("percent", percent);
		proxy.fireEvent(EVENT_BUFFERING, data);
		// Remove this after 100 full
			...
			public void run() {
				if (mp != null && mp.isPlaying()) {
					...
					if(position > duration){
						...
						Log.d(LCAT, "STREAMING..  stopping timer");
						streaming = true;
	...
	public void propertyChanged(String key, Object oldValue, Object newValue, KrollProxy proxy)
	{
		...
		//	Log.i(LCAT, "PROPERTY_URL CHANGED : url: "+TiConvert.toString(proxy.getProperty("url")));
		//	setProperty(TiC.PROPERTY_URL, resolveUrl(null, TiConvert.toString(url)));			
		//	release();
		//	getSound();
		//	Log.i(LCAT, "setUrl : url: " + TiConvert.toString(getProperty("url")));
	...
	{
		// To match TiAVPlayer iOS module.
		KrollDict data = new KrollDict();
